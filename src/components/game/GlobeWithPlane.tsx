...
const AirplanePath = () => {
  ...
  useFrame(({ clock }) => {
    if (planeRef.current) {
      ...
      const y = Math.sin(time * 0.75) * 0.3; // Add some vertical movement
      ...
      planeRef.current.lookAt(
        Math.sin(time + 0.1) * pathRadius,
        Math.sin((time + 0.1) * 0.75) * 0.3,
        Math.cos(time + 0.1) * pathRadius
      );
    }
  });
  ...
  return (
    <group ref={planeRef}>
      <mesh>
        ...
        <meshStandardMaterial color="#E31937" />
      </mesh>
      {/* Wings */}
